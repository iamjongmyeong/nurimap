...
- 프론트엔드 구현 작업인 경우 `.codex/skills/`의 `vercel-react-best-practices`와 `frontend-design` 기준을 적용했다.
- 프론트엔드 UI 리뷰나 UX 점검 작업인 경우 `.codex/skills/`의 `web-design-guidelines` 기준을 참고했다.
...
- 허용 도메인, 이메일 OTP, 세션 정책 등 인증 규칙이 spec과 일치한다.

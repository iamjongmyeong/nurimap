...
이 문서는 Nurimap의 전역 개발 기준과 실행 순서를 함께 관리하는 통합 문서다.
...
## Plan 8. 인증과 접근 제어
- goal
  - 사내 전용 서비스 조건을 만족하도록 로그인 링크 인증과 보호 접근 제어를 붙인다.
- included specs
...
## Plan 9. 별점 기능
- goal
  - 로그인 사용자의 별점 기능을 완성한다.
- included specs
  - `docs/specs/09-rating.md`
- expected outcome
  - 사용자가 place에 별점을 남기고 수정할 수 있다.
  - 목록과 상세의 평균 별점이 갱신된다.

## Plan 10. 리뷰와 추천
- goal
  - 로그인 사용자의 리뷰와 추천 기능을 완성한다.
- included specs
  - `docs/specs/10-review.md`
  - `docs/specs/11-recommendation.md`
- expected outcome
  - 사용자가 리뷰를 남기고 추천을 추가/취소할 수 있다.
...
## Spec Writing Rule
- spec 문서 하나는 하나의 기능 또는 task만 담당한다.
- 각 spec 문서는 목표, 범위, 구현 규칙, acceptance criteria, TDD 순서, 테스트 케이스, 수동 QA를 포함한다.
- AI Agent는 구현 전에 관련 spec을 읽고 failing test를 먼저 만든 뒤 구현해야 한다.

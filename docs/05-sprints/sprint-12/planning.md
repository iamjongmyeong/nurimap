...
- 로그인 메일 본문은 가운데 정렬된 `NURIMAP LOGIN` 브랜드 텍스트, `누리맵에 로그인하려면 아래 버튼을 클릭해주세요 👇` 카피, 로그인 버튼, 5분 유효 안내, raw URL fallback을 포함한다.
...
- 수신된 메일 본문에서 가운데 정렬 카피, 로그인 버튼, 5분 유효 안내, raw URL fallback이 보이는지 확인한다.

...
- `장소 추가` 버튼을 누르면 place 추가 UI를 연다.

### Detail State
- 장소 목록 페이지 또는 지도에서 place 선택 시 상세 화면을 연다.
...
- 지도 화면으로 돌아갈 때 선택한 place를 유지한다.
- 지도 화면으로 돌아갈 때 지도는 선택한 place 위치를 기준으로 보여준다.
...
| `place_lookup` | `idle`, `validating_url`, `loading`, `error` | place 추가 UI | Naver Map URL 검증, 조회, 좌표 확보 상태 |
| `place_submit` | `idle`, `submitting`, `error` | place 추가 UI | place 저장 상태 |
| `place_list_load` | `idle`, `loading`, `empty`, `ready`, `error` | 목록 화면 | place 목록 로딩 상태 |
| `place_detail_load` | `idle`, `loading`, `ready`, `error` | 상세 화면 | place 상세 로딩 상태 |
...
- place 조회 실패와 좌표 확보 실패는 브라우저 기본 `alert`가 아니라 modal로 표시한다.
- place 조회 실패와 좌표 확보 실패 시 사용자가 입력한 URL은 유지한다.
- place 등록 저장 실패 시 사용자가 입력한 `place_type`, `zeropay_status`, 별점, 리뷰는 유지한다.
- place 등록 저장 중에는 제출 버튼을 비활성화한다.
- place 등록 성공 시 별도 완료 화면 대신 결과 place 상세 화면을 연다.
- place 등록의 초기 별점과 리뷰 입력 UI는 현재 사용자가 해당 place에 review가 없을 때만 표시한다.
- place 상세의 `내 별점 상태`는 현재 사용자의 단일 review `rating_score`를 기준으로 표시한다.
- 현재 사용자가 이미 같은 place에 review를 작성한 경우 리뷰 작성 UI를 노출하지 않는다.
- 현재 사용자가 이미 같은 place에 review를 작성한 경우 기존 내 리뷰와 내 별점 상태를 보여준다.
...
- 모바일 상세는 modal이나 bottom sheet가 아니라 전체 화면 페이지로 표시한다.
- 모바일 상세의 화면 뒤로 가기 버튼과 브라우저 기본 뒤로 가기는 같은 동작을 해야 한다.
- 모바일 상세에서 지도 화면으로 복귀할 때 선택한 place는 유지한다.
- 모바일 상세에서 지도 화면으로 복귀할 때 지도는 선택한 place 위치를 기준으로 보여준다.

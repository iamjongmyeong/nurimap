...
- 인증 실패 화면은 브랜드 영역을 상단에 다시 노출하고, `인증에 실패했어요 🥲` 제목 + 본문 + 세로 CTA 구조를 사용한다.
- 인증 실패 화면에는 `새 링크 받기`와 `이메일 다시 입력` CTA를 함께 둔다.
- 인증 실패 이유는 raw code(`expired`, `used`, `invalidated`)를 그대로 보여주지 않고, `로그인 링크가 만료됐어요.\n새 로그인 링크를 받아주세요.`, `이미 사용한 링크예요.\n새 로그인 링크를 받아주세요.`, `로그인 링크가 만료됐어요.\n새 로그인 링크를 받아주세요.` 중 하나로 노출한다.

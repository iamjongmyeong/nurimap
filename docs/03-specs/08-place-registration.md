...
- 데스크톱에서는 사용자가 별 아이콘을 클릭하거나 hover하면 점수가 즉시 변경된다.
- 모바일에서는 사용자가 별 아이콘을 클릭 또는 터치하면 점수가 즉시 변경된다.
- 예를 들어 4점을 선택하면 활성 별 4개와 비활성 별 1개가 보인다.
- 후기 입력 필드는 최소 높이 `88px`를 유지하고, 입력 줄 수가 늘어나면 내용에 맞춰 높이가 커진다.
- 후기는 500자 이하로 제한한다.
...
- 저장 중에는 제출 버튼을 비활성화하고 버튼 텍스트 `등록 중`과 spinner를 함께 보여준다.
- 저장 중에는 별도 안내 박스를 추가로 보여주지 않는다.
- 저장 실패 시 browser alert으로 `등록하지 못했어요.\n\n잠시 후 다시 시도해 주세요.`를 보여주고 등록 화면에 머문다.
...
- 후기 입력 줄 수가 많아지면 field 높이가 내용에 맞춰 늘어난다.
- 초기 별점은 기본적으로 5점 선택 상태다.
- 데스크톱에서는 사용자가 별을 hover하거나 클릭하면 점수가 변경된다.
- 모바일에서는 사용자가 별을 클릭/터치하면 점수가 변경된다.
- 잘못된 enum 값은 저장되지 않는다.
- 후기 500자 초과 입력 시 초과분이 버려지고 500자까지만 유지된다.
- 좌표 없는 place는 저장되지 않는다.
- geocoding 실패 시 browser alert(`주소를 찾지 못했어요.\n\n입력한 주소를 다시 확인해 주세요.`)과 주소 필드 inline error가 함께 보인다.
...
- 저장 중에는 버튼 텍스트 `등록 중`과 spinner가 함께 보인다.
...
5. 데스크톱 별점 hover / 클릭 변경 테스트를 작성한다.
6. 모바일 별점 클릭 변경 테스트를 작성한다.
7. 후기 길이 clamp 테스트를 작성한다.
8. 후기 textarea auto-grow 테스트를 작성한다.
9. 좌표 없는 등록 차단 테스트를 작성한다.
10. 초기 리뷰 생성 테스트를 작성한다.
11. geocoding 실패 browser alert + inline error 테스트를 작성한다.
12. 중복 장소 confirm 표시 테스트를 작성한다.
13. 중복 장소 confirm `확인` 처리 테스트를 작성한다.
14. 중복 장소 confirm `취소` 유지 테스트를 작성한다.
15. 기존 review overwrite 테스트를 작성한다.
16. 후기 비어 있음 시 평가만 갱신 테스트를 작성한다.
17. dirty state 뒤로가기/닫기 confirm 테스트를 작성한다.
...
- 평가는 별 아이콘 5개로 보인다.
- 기본 상태에서 5점이 선택되어 있다.
- 데스크톱에서 4번째 별을 hover하면 4점으로 변경된다.
- 4점을 선택하면 활성 별 4개와 비활성 별 1개가 보인다.
- 후기 입력 필드는 다른 입력보다 높게 보인다.
- 후기 입력 줄 수가 많아지면 textarea 높이가 커진다.
...
- 저장 중에는 제출 버튼 텍스트만 `등록 중...`로 바뀌고 별도 안내 박스는 보이지 않는다.

...
- cooldown에 걸린 요청은 로그인 화면 또는 OTP 입력 화면에서 처리하고, 남은 대기 시간을 사람이 이해하기 쉬운 형식으로 보여준다.
- OTP 요청 성공 후에는 페이지 이동 없이 현재 auth 흐름 안에서 OTP 입력 상태를 표시한다.
- OTP 입력 상태에는 사용자가 입력한 이메일 주소와 재전송 CTA가 함께 보여야 한다.
...
- wrong code 사용자 문구는 `인증 코드가 올바르지 않아요. 다시 확인해 주세요.`를 사용한다.
- expired code 사용자 문구는 `인증 코드가 만료됐어요.\n새 코드를 받아주세요.`를 사용한다.
- invalidated code 사용자 문구는 `새 코드가 발급됐어요.\n새 코드를 입력해 주세요.`를 사용한다.

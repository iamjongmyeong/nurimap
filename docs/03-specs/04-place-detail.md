...
## Summary
place 상세 화면에 표시해야 할 정보, canonical detail route, 상세 화면 동작, detail 하단 review CTA/add-rating 전환을 정의한다.
...
- 상세 로딩/오류 상태
- detail 하단 `평가 남기기` CTA와 add-rating child surface
...
- 현재 사용자에게 `my_review === null`이면 detail 하단에 `평가 남기기` CTA를 표시한다.
- 현재 사용자에게 `my_review !== null`이면 새 review 작성용 CTA나 composer entry를 표시하지 않는다.
- `평가 남기기` CTA는 detail-owned add-rating child surface를 연다.
...
- 승인된 add-rating surface 외의 별도 detached review action surface는 기본 범위에 포함하지 않는다.
...
- `my_review === null`인 detail에서만 `평가 남기기` CTA가 보인다.
- `my_review !== null`인 detail에서는 CTA가 보이지 않는다.
- CTA 진입 시 add-rating child surface가 열린다.
...
9. detail CTA visible/hidden 테스트를 작성한다.
...
- `my_review === null` detail의 CTA visible
...
- `my_review === null`인 place에서만 `평가 남기기` CTA가 보인다.
- CTA 진입 후 add-rating에서 back/cancel 시 같은 detail로 돌아간다.

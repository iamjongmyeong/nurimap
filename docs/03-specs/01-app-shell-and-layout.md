...
- `/add-place` 내부 URL-entry → manual-form staged flow

## Out Of Scope
- 로그인 구현
- place 저장
- 실제 외부 API 연동
- review domain rule와 집계 정책 세부
...
3. `/add-place` URL-entry first step 테스트를 작성한다.
4. 모바일 하단 고정 3탭 바 레이아웃 테스트를 작성한다.
5. 모바일 `목록` 탭 이동 테스트를 작성한다.
6. 모바일 detail full-screen 레이아웃 테스트를 작성한다.
7. 모바일 add-rating child surface 상태 전환 테스트를 작성한다.
8. 빈 상태/로딩 상태 테스트를 작성한다.
9. 구현한다.
10. 전체 테스트를 통과시킨다.
...
- `/add-place` URL-entry first step 렌더링
- 모바일 하단 고정 3탭 바 렌더링
- 모바일 `목록` 탭 이동
...
- `+` 탭에서 `/add-place` 진입 시 URL-entry step이 먼저 보인다.

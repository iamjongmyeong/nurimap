...
3. 모바일에서는 목록 보기로 장소 목록으로 이동하거나 지도 마커를 클릭한다.
4. 목록에서는 평균 별점, 리뷰 수, 제로페이 가능 여부를 함께 확인한다.
5. 장소를 선택하면 공유 가능한 상세 상태로 진입한다.
6. 데스크톱에서는 목록 영역 안의 내용이 상세 화면으로 전환된다.
7. 모바일에서는 상세 화면이 전체 화면으로 열린다.
8. 현재 사용자가 해당 place를 아직 평가하지 않았다면 detail 하단의 `평가 남기기` CTA로 add-rating surface를 연다.
9. add-rating에서 저장하거나 뒤로 가면 사용자는 같은 place detail로 돌아간다.
10. 사용자가 `장소 추가`를 누르면 기존 목록 영역이 등록 화면으로 바뀐다.
11. 등록 화면을 닫으면 사용자는 직전의 목록 탐색 맥락으로 돌아간다.
12. 상세에서 평균 별점, 별점 수, 장소 구분, 제로페이 가능 여부, 장소를 추가한 사람 이름, 리뷰를 본다.
13. 리뷰와 평가는 가장 최근 항목부터 위에서 아래 순서로 본다.
14. 데스크톱에서는 닫기 액션으로, 모바일에서는 뒤로 가기 액션 또는 브라우저 기본 뒤로 가기로 탐색 화면으로 돌아간다.
